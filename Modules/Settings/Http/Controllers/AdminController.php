...
class AdminController extends BaseAdminController
{
    ...
    public function save(Request $request, FileUploader $fileUploader): RedirectResponse
    {
        ...
        if ($request->hasFile('footer_logo')) {
            $file = $fileUploader->handle($request->file('footer_logo'), 'settings');
            $this->deleteImageFooterLogo();
            $data['footer_logo'] = $file['filename'];
        }
    ...
    public function deleteImageFooterLogo(): void
    {
        if ($filename = Setting::where('key_name', 'footer_logo')->value('value')) {
            try {
