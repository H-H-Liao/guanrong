<template>
    <div class="item-list">
        ...
        <category-list v-if="category" :url="this.urlBase" @onCategoryItemClick="itemClick" :select="selectItem"></category-list>
    ...
    methods: {
        ...
        fetchData() {
            ...
            if(this.selectItem>0){
                $newUrl=$newUrl+'&category_id='+this.selectItem;
            }
            console.log($newUrl);
            this.startLoading();
            axios
        ...
        copyItem() {
            console.log('複製v6');
              axios
                .all(
                    this.checkedItems.map((model) =>
                         axios.post(this.urlBase + '/' + model.id + '/copy')
                        .catch((error) =>
                ...
                .then((responses) => {
                    ...
                    let successes = responses.filter((response) => response.data.statusText === 'OK');
                    console.log(successes);
                    if (successes.length > 0) {
                        alertify.success(
                            this.$i18n.tc('複製成功', successes.length, {
                                count: successes.length,
                            })
                        );
                        console.log('ok');
